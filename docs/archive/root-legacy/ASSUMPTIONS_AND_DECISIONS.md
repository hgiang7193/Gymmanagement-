...
| A-008 | AI nutrition advisor trong Phase 2 chi generate meal plans dua tren weight metrics, khong phai medical advice. | Giam rui ro compliance va liability; tap trung vao wellness guidance. | Neu user xem nhu medical advice, can them disclaimers ro rang. | TBD | Khi launch nutrition feature |
| A-009 | Meal plans duoc generate 1 lan/ngay va luu lai; khong regenerate lien tuc de tranh AI cost cao. | Kiem soat chi phi API calls va dam bao consistency cho user. | User muon refresh plan giua ngay se bi han che. | TBD | Khi co feedback tu users |
| A-010 | Nutrition data (meal plans, consumption logs) duoc xu ly voi same privacy level nhu health metrics. | Compliance voi COMP-01, COMP-02; health-related data can protection. | Leak nutrition data co the vi pham privacy. | TBD | Khi implement RBAC cho nutrition context |
...
| D-006 | AI nutrition advisor su dung external LLM API (DeepSeek/GPT-4/Claude) thay vi self-hosted model trong Phase 2. | Giam complexity infrastructure; faster time-to-market. | Phu thuoc vao third-party API availability va cost. | Khi scale len hoac can data sovereignty cao hon.
...
| Q-005 | AI nutrition advisor nen su dung LLM provider nao (DeepSeek, GPT-4, Claude)? Cost vs accuracy trade-off? | Architecture decision, cost planning | Before Phase 2 implementation |
| Q-006 | Nutrition goals/macros targets nen duoc set boi user hay AI recommend? | User experience design, AI prompt strategy | Before nutrition feature design |
