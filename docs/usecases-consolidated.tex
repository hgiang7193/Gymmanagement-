...
Bản đặc tả này được hợp nhất từ phiên bản 1.0 (~40 UC) thành \textbf{18 UC} dựa trên 7 nguyên tắc sau, nhằm tránh chia nhỏ vô nghĩa và phản ánh đúng định nghĩa kinh điển về use case:
...
Huấn luyện viên & 2 & UC-COACH-01 Quản lý ca làm việc · UC-COACH-02 Hỗ trợ check-in lỗi \\ \hline
...
\multicolumn{2}{|r|}{\textbf{Tổng}} & \textbf{18 use case} \\ \hline
\end{tabular}
\caption{Tổng quan 18 use case sau khi gộp}
\end{table}
...
%% UC-COACH-01
\subsubsection{UC-COACH-01: Quản lý ca làm việc của HLV}
\begingroup
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{|>{\centering\arraybackslash}m{3cm}|>{\raggedright\arraybackslash}m{12cm}|}
\hline
...
\hline
\endfoot
\textbf{Mục đích} & HLV tự đăng ký các ca trống tại branch của mình và chạy các ca đã đăng ký (xem lịch, xem roster, xem thông tin học viên). \emph{Gộp toàn bộ vòng đời ca từ phía HLV} — đăng ký + chạy ca. Manager KHÔNG phân ca cho HLV trong mô hình này; HLV chủ động chọn lịch làm việc. \\
\hline
\textbf{Tác nhân} & Coach. \\
\hline
\textbf{[ĐK]} & Coach đã đăng nhập, thuộc branch có ca, ca chưa đủ HLV (cho luồng đăng ký). \\
\hline
\textbf{[KT]} & \emph{Đăng ký:} bản ghi \texttt{trainer\_assignments} được tạo, audit \texttt{coach\_self\_assigned}. \emph{Chạy ca:} HLV thấy được lịch, roster real-time và thông tin tối thiểu của từng học viên (least privilege). \\
\hline
\textbf{Luồng chính} & \textbf{(a) Đăng ký ca:} 1) HLV mở ``Ca trống'' $\rightarrow$ thấy các ca chưa đủ HLV trong branch và khoảng thời gian. 2) Chọn ca, xác nhận. 3) Hệ thống validate (không trùng giờ với ca đã nhận, còn slot HLV) $\rightarrow$ tạo assignment. 4) HLV có thể tự huỷ assignment trước khi ca bắt đầu. \textbf{(b) Chạy ca:} 1) HLV mở màn hình chính $\rightarrow$ hệ thống lấy ca theo ngày từ \texttt{trainer\_assignments}. 2) Chọn ca $\rightarrow$ hiển thị roster real-time (\texttt{class\_attendance}): tổng check-in, tên, giờ check-in, sinh trắc. 3) Click học viên $\rightarrow$ hiển thị tên, ảnh, cân nặng hôm nay, số buổi còn lại. \\
\hline
\textbf{[NL]} & \textbf{NL-1:} Đăng ký ca trùng giờ $\rightarrow$ \texttt{SHIFT\_TIME\_OVERLAP}. \textbf{NL-2:} Ca đã đủ HLV $\rightarrow$ \texttt{SHIFT\_FULL}. \textbf{NL-3:} Huỷ ca sau khi đã bắt đầu $\rightarrow$ \texttt{SHIFT\_ALREADY\_STARTED}. \textbf{NL-4:} Truy cập ca không cùng branch $\rightarrow$ \texttt{FORBIDDEN}. \\
\hline
\end{longtable}
\endgroup

%% UC-COACH-02
\subsubsection{UC-COACH-02: Hỗ trợ học viên check-in lỗi}
\begingroup
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{|>{\centering\arraybackslash}m{3cm}|>{\raggedright\arraybackslash}m{12cm}|}
\hline
...
\hline
\endfoot
\textbf{Mục đích} & Đảm bảo dữ liệu attendance chính xác khi member không thể tự check-in (sự cố app/cân, ngoài cửa sổ, hết grace period). Phân biệt với UC-COACH-02: manager có thẩm quyền rộng hơn (grace period, mọi ca trong branch). \\
\hline
\textbf{Tác nhân} & Manager (branch scope). \\
\hline
...
\hline
\textbf{Luồng chính} & \textbf{1.} Manager nhận thông báo từ member (UC-MEMBER-01) hoặc HLV (UC-COACH-02). \textbf{2.} Xác minh danh tính + tình trạng có mặt. \textbf{3.} Chọn member + ca, nhập lý do bắt buộc. \textbf{4.} Hệ thống tạo attendance + audit trong transaction. \\
\hline
